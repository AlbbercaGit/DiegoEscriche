  ...
  return (
    <header className={`flex justify-center ${isOpen ? 'bg-transparent' : 'bg-transparent'} text-black font-sans relative z-10`}>
      <div className="absolute w-4/5 h-[8vh] container mx-auto flex items-center justify-between md:relative">
        ...
        >
          <span
            className={`block w-6 h-0.5 bg-black text-black transform transition-transform duration-300 ease-in-out ${isOpen ? 'rotate-45 translate-y-1.5' : ''}`}
          />
          <span
            className={`block w-6 h-0.5 bg-black text-black my-1 transition-opacity duration-300 ease-in-out ${isOpen ? 'opacity-0' : ''}`}
          />
          <span
            className={`block w-6 h-0.5 bg-black text-black transform transition-transform duration-300 ease-in-out z-20 ${isOpen ? '-rotate-45 -translate-y-1.5' : ''}`}
          />
        </button>
        ...
        <nav className="hidden md:flex md:items-center">
          <ul className="flex space-x-4">
            ...
            <li><a href="#about" className="hover:underline">Proyectos</a></li>
            <li><a href="#contact" className="hover:underline">Mezclas</a></li>
          </ul>
        </nav>
      </div>
        ...
        <div className="relative w-full h-full flex items-center justify-center text-white">
          ...
          <nav className="flex flex-col items-center text-center text-2xl">
            <ul className="space-y-4">
              <li><a href="#home" className="text-3xl font-sans py-2 px-4 underline-button" onClick={toggleMenu}>Home</a></li>
